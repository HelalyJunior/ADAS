...
def warp_perspective(img,mat):
    return cv2.warpPerspective(img ,mat ,(800,600))
...
input_top_left = [340,330]
input_top_right = [430,330]
input_bottom_left = [150,580]
input_bottom_right = [680,580]
...
dstPts = np.float32([[0,600],[0,0],[800,0],[800,600]])
...
while counter<1622:
  ...
  bird_temp_cpy = bird_cpy.copy()

  # ENLIGHTEN THE IMAGE QUITE A BIT FOR BETTER DETECTION !
  lightened = adjust_gamma(bird_view,0.95)

  # CREATE A YELLOW MASK AND A WHITE MASK THEN GET A MASK CONTAINING BOTH
  white_mask = isolate_color_mask(to_hls(lightened), np.array([0, 225, 0], dtype=np.uint8), np.array([200, 255, 255], dtype=np.uint8))
  ...
  dilated_edges = cv2.dilate(edges,np.ones((50,50)))
  ...
  for ctr in contours:
    ...
    if cv2.contourArea(ctr)<8000:
      continue
    list_of_pts += [pt[0] for pt in ctr]
  ctr = np.array(list_of_pts).reshape((-1,1,2)).astype(np.int32)
  ctr = cv2.convexHull(ctr)
  cv2.fillConvexPoly(bird_temp_cpy, ctr,color=(0, 0, 255))

  # i added this
  # cv2.drawContours(bird_temp_cpy, contours, -1, (0, 0, 255),thickness=cv2.FILLED)


  # cv2.drawContours(bird_temp_cpy, c, -1, (0, 0, 255), thickness=cv2.FILLED)
  ...
  #   if left[0] > 1.4*medx or left[1] > medy :
  #     points[3]= old_pts[3]
  #     left= old_pts[3]
  #   else:
  #     old_pts[3]=left
  ...
  for c in contours :
    ...
    cv2.rectangle(bird_temp_cpy,(x,y),(x+w,y+h),(0,0,255),-1)
...
# REGENERATE THE IMAGE TO THE NORMAL PRESPECTIVE 
  ...
  re_gen = warp_perspective(bird_temp_cpy,mat_inv)
