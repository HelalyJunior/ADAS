...
def extract_white(image):
    lower_thresh = np.array((100,100,100), dtype = np.uint8)
    upper_thresh = np.array((255,255,255), dtype = np.uint8)

    # image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    white_mask = cv2.inRange(image, lower_thresh, upper_thresh)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    print(white_mask.shape)
    white_pixels_image = cv2.bitwise_and(image, white_mask, mask = None)
    return white_mask
def detect_edges(image , s_thresh , l_thresh , shad_thresh= (50,155)):

    # image = extract_white(image)
...
def sliding_window(img,dst_colored, window_size , right_poly_old,left_poly_old, frame_discard):
    ...
    start_left_x= np.argmax(histogram[:midpoint - 100]) - 90
    ...
    for window in range(nwindows):
        ...
        cv2.rectangle(dst_colored,(left_lane_lbound,win_bot),(left_lane_rbound,win_top),(0,0,255), 3) 
        cv2.rectangle(dst_colored,(right_lane_lbound,win_bot),(right_lane_rbound,win_top),(0,0,255), 3) 
        ...
        if len(good_left_inds) > 40:
            start_left_x = int(np.mean(white_x[good_left_inds]))
        if len(good_right_inds) > 40:        
    ...
    # print(f"Number of left lane white pixels detected is: {len(leftx)}\n")
    ...
    if len(leftx) >= 1200:
        ...
        left_fitx = left_poly_old
        right_fitx = right_poly_old
    
    left_poly = np.asarray(tuple(zip(left_fitx,ploty)) ,np.int32)
    right_poly = np.asarray(tuple(zip(right_fitx,ploty)),np.int32)

    # if not(right_poly_old is None) and abs(right_poly_old[30][0] - right_poly[30][0]) > frame_discard:
    #     right_poly = right_poly_old
    # if not(left_poly_old is None) and abs(left_poly_old[30][0] - left_poly[30][0]) > frame_discard:
...
def calc_off_dist(M,frame, right_lane_pts, left_lane_pts):                     #Calculating distance off center
        
        centre_car = (frame.shape[1]//2, 600)                                #determining center of car from original view
